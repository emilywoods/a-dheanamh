...
def insert_to_db(input: List):
    raw = [tuple(record.values()) for record in input]

    with connect("http://localhost:4200/", username="", password="") as conn:
...
def main(args) -> None:
    ...
    completed_tasks = list(filter(lambda task: task['idList'] == list_id, list_of_cards))
